...
from sqlalchemy import func, cast, Interval, or_
...
def delete_expired_consumers():
    ...
    with app.app_context():
        ...
        subscriptions = db.session.query(ApiServiceSubscription).filter(
            func.now() > (
                ApiServiceSubscription.start_date +
                cast(func.concat(
                    ApiServiceSubscription.duration_month, ' month'), INTERVAL)
            ),
            or_(
